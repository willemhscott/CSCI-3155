  ...
  def evalExpr(e: Expr, env: Environment): Value = e match {
    ...
    case Plus(e1, e2) =>
      (evalExpr(e1, env), evalExpr(e2, env)) match {
        ...
        case _ => throw new IllegalArgumentException("Invalid operands for +")
      }
    case Minus(e1, e2) =>
      (evalExpr(e1, env), evalExpr(e2, env)) match {
        case (NumValue(a), NumValue(b)) => NumValue(a - b)
    ...
    case Div(e1, e2) =>
      (evalExpr(e1, env), evalExpr(e2, env)) match {
        case (NumValue(a), NumValue(b)) => NumValue(a / b)
        case (FigValue(f1), NumValue(a)) =>
          val (xmin, xmax, ymin, ymax) = f1.getBoundingBox
          FigValue(f1.translate(-xmax, -ymin).rotate(a).translate(xmax, ymin))
    ...
    case FunCall(fCall, arg) => // copied from hw
      evalExpr(fCall, env) match {
        case Closure(x, closure, env2) => evalExpr(closure, Extend(x, evalExpr(arg, env), env2))
        case _ => throw new IllegalArgumentException("Invalid call operands")
      }
  }
